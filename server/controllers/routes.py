...
from server.utils.models import route, error_response
...
@routes_ns.route('/')
@routes_ns.doc(description='Gives Directions with Safety Ratings from a given Origin to a Destination', params=route_input)
class Route(Resource):
    ...
    @routes_ns.marshal_list_with(route, code=200)
    @routes_ns.marshal_with(error_response, code=400)
    @routes_ns.marshal_with(error_response, code=401)
    @sanitize({'origin_latitude': None, 'origin_longitude': None, 'dest_latitude': None, 'dest_longitude': None})
    def get(self):
        gmap_request = get( 
            self.directions(
